...
test('shemiratHaLashon-3-Av', () => {
  ...
  expect(reading1).toEqual({bk: 2, k: 'x', b: 16.14, e: 16.15});
  expect(reading2).toEqual({bk: 2, k: 'x', b: 19.5, e: 19.6});
});
...
test('ShemiratHaLashonEvent', () => {
  const hd = new HDate(23, 'Adar I', 5784);
  const reading = shemiratHaLashon(hd);
  const ev = new ShemiratHaLashonEvent(hd, reading);
  expect(ev.getDesc()).toBe('Book I, Shar Hatorah 2.20-2.23');
  expect(ev.memo).toBe('Book I, The Gate of Torah 2.20-2.23');
  expect(ev.url()).toBe('https://www.sefaria.org/Shemirat_HaLashon%2C_Book_I%2C_The_Gate_of_Torah.2.20?lang=bi');
});
